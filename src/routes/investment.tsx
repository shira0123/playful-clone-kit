    ...
    <PageLayout>
      ...
      <section className="py-20 px-4">
        <div className="mx-auto max-w-6xl grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((p) => (
            <div key={p.name} className={`p-8 rounded-lg border ${p.popular ? "border-gold shadow-xl bg-navy text-white" : "bg-white"}`}>
              {p.popular && <span className="inline-block mb-3 text-xs uppercase tracking-widest text-gold font-semibold">Most Popular</span>}
              ...
              <a href="https://app.e-directpro.com/register" className={`mt-8 block text-center rounded-md py-3 font-semibold ${p.popular ? "bg-gold text-navy" : "bg-navy text-white"}`}>Invest Now</a>
            </div>
          ))}
        </div>
      </section>
